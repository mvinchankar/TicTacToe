...
function play()
{
     ...
     read -p "Enter the Position Show Your Skills: " position
     if [[ position -le 9 ]] && [[ position -ne 0 ]]
     then
          if  [[ ${boardOfTicTacToe["$position"]} -eq "$player" ]] || [[ ${boardOfTicTacToe["$positionOfComputer"]} -eq "$computer" ]]
          then 
              ...
              if [[ $resultOfToss -eq 1 ]]
              then 
                  boardOfTicTacToe[$positionOfComputer]=$computer
                  resultOfToss=0;
                  displayBoard
